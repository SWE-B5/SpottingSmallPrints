...
#Schluessel Drop
func _on_interact():
	print(PlayerVariables.collected_golden_keys.find(id))
	# Logik muss noch angepasst werden wenn die neuen Scripts für Variablen im Master sind
	if is_closed && PlayerVariables.collected_golden_keys.find(id) != -1:
